...
/** A ClassLoader that reads classes from a Hadoop FileSystem or Spark RPC
  * endpoint, used to load classes defined by the interpreter when the REPL is
  * used. Allows the user to specify if user class path should be first. This
  * class loader delegates getting/finding resources to parent loader, which
  * makes sense because the REPL never produce resources dynamically. One
  * exception is when getting a Class file as resource stream, in which case we
  * will try to fetch the Class file in the same way as loading the class, so
  * that dynamically generated Classes from the REPL can be picked up.
  *
  ...
  * using inappropriate class loader, we should set the parent of ClassLoader to
  * null, so that we can fully control which class loader is used. For detailed
...
class ExecutorClassLoader(
  ...
  val absolutePath = {
    if (uri.getPath().isEmpty()) "/" else uri.getPath()
  }
  ...
  private def getClassFileInputStreamFromFileSystem(
    ...
    val path = new Path(new Path(absolutePath), pathInDirectory)
    try {
...
/** An error when we cannot load a class due to exceptions. We don't know if
  * this class exists, so throw a special one that's neither [[LinkageError]]
  * nor [[ClassNotFoundException]] to make JVM retry to load this class later.
  */
